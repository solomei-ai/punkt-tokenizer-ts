...
describe('Performance and Memory', () => {
    test('should handle large texts efficiently', () => {
        ...
        const paragraphs = Array.from({ length: 50 }, (_, i) =>
            `This is paragraph ${i+1}. It has multiple sentences. Each with different structures!`
        ).join('\n\n');
    ...
    test('should clear caches between tokenization calls', () => {
        ...
        const nextTokensClearSpy = jest.spyOn((tokenizer as any)._cachedNextTokens, 'clear');
        const abbreviationsClearSpy = jest.spyOn((tokenizer as any)._cachedAbbreviations, 'clear');
