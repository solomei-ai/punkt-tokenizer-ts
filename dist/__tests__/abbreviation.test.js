...
describe("Abbreviation Handling", () => {
    ...
    test("should not split on common abbreviations", () => {
        const result = tokenizer.tokenize("Dr. Smith went to Washington D.C. He was happy.");
    ...
    test("should handle abbreviations followed by sentence starters", () => {
        const result = tokenizer.tokenize("She visited the U.S. The president greeted her.");
        expect(result).toHaveLength(2);
    });
    test("should cache abbreviation results for performance", () => {
        // spy private method
        const tokenizer = new PunktTokenizer("en");
        const isAbbreviationSpy = jest.spyOn(tokenizer, "isAbbreviation");
        // repeated abbreviations
        tokenizer.tokenize("Dr. Smith and Dr. Jones both work at St. Mary hospital.");
    ...
    test("should handle cache eviction when cache gets too large", () => {
        ...
        const longText = Array.from({ length: 1500 }, (_, i) => `unique${i}. `).join("");
        const cacheSpy = jest.spyOn(tokenizer._cachedAbbreviations, "set");
        tokenizer.tokenize(longText);
        expect(tokenizer._cachedAbbreviations.size).toBeLessThan(1500);
